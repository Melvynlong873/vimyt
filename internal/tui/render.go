...
func (a App) View() string {
	...
	if a.gotoActive {
		bottomBar = renderInputWithNowPlaying(a.gotoInput.View(), a.player.Status(), a.width, a.autoplay, a.shuffle)
	} else if a.colonActive {
		bottomBar = renderInputWithNowPlaying(a.colonInput.View(), a.player.Status(), a.width, a.autoplay, a.shuffle)
		...
		bottomBar = renderNowPlaying(a.player.Status(), a.width, favSet, a.autoplay, a.shuffle, a.tickCount)
	}
	...
	// Compute left column split: Playlists, History, and Radio History heights
	...
	if a.showHistory && a.showRadio {
		// 3 panels: Playlists, Play History, Radio History
		if plExpanded {
			// Playlist viewing tracks: give it bulk, others compact
			historyH = compactH
			radioHistH = compactH
			if radioHistExpanded {
				radioHistH = bottomH / 3
				radioHistH = max(radioHistH, 5)
			}
			playlistH = bottomH - historyH - radioHistH
			...
			historyH = bottomH - playlistH - radioHistH
			if historyH < 5 {
				historyH = 5
				playlistH = bottomH - historyH - radioHistH
			}
			...
			playlistH = plListH
			historyH = compactH
			radioHistH = bottomH - playlistH - historyH
			if radioHistH < 5 {
				radioHistH = 5
				playlistH = bottomH - historyH - radioHistH
			}
			...
			historyH = compactH
			radioHistH = compactH
			remaining := bottomH - playlistH - historyH - radioHistH
			if remaining > 0 {
		...
		if plExpanded {
			historyH = compactH
			playlistH = bottomH - historyH
		} else if a.focusedPanel == panelHistory {
			playlistH = plListH
			historyH = bottomH - playlistH
			if historyH < 5 {
				historyH = 5
				playlistH = bottomH - historyH
			}
			...
			playlistH = plListH
			historyH = compactH
			remaining := bottomH - playlistH - historyH
			if remaining > 0 {
				playlistH += remaining
			} else {
				playlistH = bottomH - historyH
		...
		if plExpanded {
			...
			playlistH = bottomH - radioHistH
		} else if a.focusedPanel == panelRadioHist || radioHistExpanded {
			playlistH = plListH
			radioHistH = bottomH - playlistH
			if radioHistH < 5 {
				radioHistH = 5
				playlistH = bottomH - radioHistH
			}
			...
			playlistH = plListH
			radioHistH = compactH
			remaining := bottomH - playlistH - radioHistH
			if remaining > 0 {
				playlistH += remaining
			} else {
				playlistH = bottomH - radioHistH
	...
	// Ensure left column panel heights sum exactly to bottomH.
	// Without this, clamping individual panels can cause the left column to
	// exceed bottomH, making the total View output taller than the terminal
	// and causing the TUI to scroll.
	if a.showHistory && a.showRadio {
		// 3 panels: absorb any surplus/deficit into playlistH
		playlistH = bottomH - historyH - radioHistH
		if playlistH < 3 {
			playlistH = 3
			// Still overflowing — shrink historyH
			historyH = bottomH - playlistH - radioHistH
			if historyH < 1 {
				historyH = 1
				radioHistH = bottomH - playlistH - historyH
	...
	// Radio History panel (shown when showRadio is true)
